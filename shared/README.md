...
- **Match conditions** use `matchType` / `matchOperator` / `matchValue` (+ optional `negate`, `caseSensitive`, `headerName`). `matchType ∈ {path, hostname, protocol, regex, header, cookie, country}`, `matchOperator ∈ {equals, contains, regex, notEquals}`, `notEquals` for `country` only. `headerName` is required iff `matchType` is `header`.
...
- **A redirect with a `country` condition cannot also have a `header`, `cookie` or `protocol` one** (the redirect schema's top-level `if`/`then`). At origin-request those only hold what the behavior's policies forward, and a dropped one reads as empty — negated, a match for everyone. The protocol falls back to `https` when `X-Forwarded-Proto` is missing, which is the same failure. Rewrites are not restricted. See [infra/lambda](../infra/lambda/README.md#what-a-geo-redirect-cannot-read).
